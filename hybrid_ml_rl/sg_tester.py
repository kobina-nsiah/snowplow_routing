...
def compute_summary_metrics(model: TwoStageRoutingModel, data_loader, main_graph, device="cpu"):
    """
...
def evaluate_model(model: TwoStageRoutingModel, data_loader, main_graph, device="cpu"):
    """
    Detailed per‐sample, per‐truck print‐out plus overall averages.
    """
    model.to(device).eval()
    all_ed, all_acc, all_cov, all_dh = [], [], [], []
    ...
    with torch.no_grad():
        ...
        for batch in data_loader:
            ...
            for i in range(len(raw_nodes_list)):
                ...
                cov = len(covered) / max(len(service_set),1) if service_set else 1.0
                ...
                sample_dh = 0.0
                for seq in routes[i]:
                    for u_idx, v_idx in zip(seq, seq[1:]):
                        edge = frozenset({nodes[u_idx], nodes[v_idx]})
                        if edge not in service_set:
                            try:
                                sample_dh += main_graph[nodes[u_idx]][nodes[v_idx]][0]["length"]
                            except Exception:
                                pass
                all_dh.append(sample_dh)
    ...
    # print(f"Avg Edit-Dist:  {sum(all_ed)/N:.4f}")
    # print(f"Avg Token-Acc:  {sum(all_acc)/N:.4f}")
    print(f"Avg Coverage:   {sum(all_cov)/len(all_cov):.4f}")
    # print(f"Avg Deadhead(m):{sum(all_dh)/len(all_dh):.1f}")
    return  # nothing to return; prints everything
